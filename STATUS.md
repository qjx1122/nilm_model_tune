...
## 下一步（TODO）
1. 等待用户新任务定向；候选方向清单见下
2. 收尾仪式（本回合完成）：REPORT.md v1.3+TUNING_GUIDE 第五纪元+session 纪要+STATUS 收官态+commit/push

**候选方向（收官后清单，供用户挑选）**：
- fridge（m14，H2 或 H1）常开型：连续负荷分解口径（事件口径失效）——须先做口径设计专题
- EE 绝对 kWh 差口径：稀疏电器（mw 类）EE 15% 线的分母效应修正——须先例分析+用户批准（不可静默改线）
- House3+ 或 REDD/Pecan Street 等跨数据集验证：五纪元结论的外部效度
- 生产化/工程化：两 house 五电器锁定配置的部署侧封装、推理侧事件解码
- Test 扩容口径（6000→更大）对 EE 分母效应的缓解评估
