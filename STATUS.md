...
## 进行中
- （用户侧）dw/wm 阈值敏感性 ×8 + mw 摸底×3 + mw f5 迁移探针×3 + evaluate ×6（命令见实录 43，~10-15min）→ 一次回传
- （本侧）无阻塞；待判读：dw 口径四判据 / wm viability 复核（500W 档 val ON<90 则排除）/ mw 迁移配对判定（显著优→免粗搜；败→独立粗搜）

## 下一步（TODO）
1. 用户：敏感性 ×8 + mw 训练 ×6 + evaluate ×6（完整命令块见 REPORT_TEST.md 实录 43）
2. 本侧判读（实录 43 预注册标准）→ dw 口径定夺+纪元锁定 → mw 路线裁定（迁移/粗搜）→ wm 去留 → 各纪元摸底/搜索/锁定/Test 预算 2 次全流程
