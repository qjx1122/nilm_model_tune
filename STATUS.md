...
## 当前目标
- 【进行中·用户任务】House2 多电器纪元群启动（实录 42 立项）：rice cooker=m9/wm=m12/dw=m13/mw=m15/toaster=m16 五纪元（fridge=m14 常开型排除本批·留口径升级后另议）；五阶段路线图（普查→口径→摸底→迁移探针优先的搜索裁定→各自 Test 预算 2 次）——当前=阶段 1 制备+身份链+形态普查 ×5（命令见实录 42/下一步块）
...
## 进行中
- （用户侧）H2 五电器普查：prepare ×5 + diagnose ×5（命令见实录 42，分钟级）→ 回传 10 份输出全文（含缺口处理行+表头行）
- （本侧）无阻塞；待身份链判读（aggOffW/corr/evt 形态/ON 相位结构）→ 口径定夺（wm/dw 预计需阈值敏感性）→ 摸底设计

## 下一步（TODO）
1. 用户：H2 五电器普查（prepare ×5 + diagnose ×5，完整命令块见 REPORT_TEST.md 实录 42；npz 命名 ukdale_h2_{ricecooker,wm,dw,mw,toaster}.nzp；mains=m1；表号 m9/m12/m13/m15/m16）
2. 本侧普查判读（身份链+形态）→ 口径定夺（周期类 wm/dw 阈值敏感性；短事件类视形态）→ 摸底 ×5 设计（baseline+val 30000+viability 口径 val ON≥90@30000）→ 迁移探针（f5_t9/d1 平移）vs 独立粗搜裁定
