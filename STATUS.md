...
## 当前目标
- 【进行中·用户任务】数据地基修复：三因修复已推送（缺口地形+选段索引 bug+sum 假零，实录 10，schema v3 全量拼接留痕）→ 待用户 git pull 重跑 prepare + diagnose，回传两份输出
...
## 进行中
- （用户侧）git pull 后重跑 prepare（同命令，覆盖 v2）+ diagnose，一次贴回两份输出（预期 n≈850-890 万、拼接数千处、days≈590-620）
- （本侧）无阻塞；判读 aggOffW/corr 定数据地基是否修复

## 下一步（TODO）
1. 用户：git pull 后重跑 prepare（同命令，覆盖 ukdale_prepared_v2.npz）→ 回传输出（含「对齐」「拼接」两行）
2. 用户：跑 diagnose_split.py --npz ukdale_prepared_v2.npz → 本侧判读（期望 aggOffW 数百 W、corr<<1、days≈590-620、壶事件回归 4-5 次/天）
