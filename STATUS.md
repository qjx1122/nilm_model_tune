...
## 进行中
- （用户侧）H2 细搜批次 2：查缺 run + 13 runs + 汇总（命令见下一步块，~25-45min GPU）→ 回传目录清单+summarize_fine 全文
- （本侧）无阻塞；待批次 2 判读（预注册判定树：f6 显著优→锁 f6；否则 top-3 n=5 分量权衡锁定，EE 近零+σ 小优先）→ Test 预注册执行（seed 9000，H2 预算 #1/2）

## 下一步（TODO）
1. 用户：查缺 run（Get-ChildItem reports\fine_h2 | Select-Object Name）→ 补 fb_basearch 缺种子 → 批次 2（引号版命令见实录 31：top-3 ×8003/8004 + f6_f5d128/f7_f5w96 ×8000-8002）→ summarize_fine
2. 本侧批次 2 判读（实录 31 预注册判定树）→ 锁定候选 → Test 预注册（seed 9000 × --test 恰一次；验收 S_test≤val 均值+0.015、F1≥0.75、R≥0.70、|EE|≤0.15；EE 方向观察量预期偏正，>+5% 如实披露）
