...
## [2026-09-09] 会话纪要（改进落盘：档案可复现条款）
- 目标：按用户批准落盘实录 21 三条改进
- 本会话角色：资深电力算法专家（默认角色，协议维护）
- 完成项：ROLE.md v1.2（两角色收尾条款增「命令+输出+判读三件套随实录落盘（命令含实录路径）」、验收标准增「档案可复现」维度，共 4 处条款+修订记录）；BOOTSTRAP.md v2.2（专题模板新增「用户执行命令」字段+版本号+修订记录）；STATUS/纪要同步；commit+push
...
## 2026-09-10 · House1 dish_washer 纪元：口径定夺 → 摸底/粗搜/细搜三批次 → d1_t11d64 锁定 → Test 四线全过收官
- 本会话角色：实验/调参教练（口径判读/搜索/锁定/Test）→ 收官卸任
- 完成项：
  - 口径定夺（实录 34-35）：阈值敏感性四跑（20/100/200/500W）+四判据预注册框架 → 200W 加热相位定义（断层在 100→200W=泵相位出局；周期结构闭环 0.4-0.5 周期/天×2 相位=0.8-1.0 vs 实测 0.91-1.02；加热占能量 84%；200/500 口径等价）→ 纪元锁定
  - 摸底（实录 36）：全门槛过 ×3（F1 0.8917±0.0231/EE −5.29%±0.29% 系统性低估/事件解码 N=130 九项闭环）；tuning_dw.yaml（窗口+384 周期探针）
  - 粗搜（实录 37）：30/32 门槛首咬（2 fail 全在 w384+d128+L1+do0 死角）；t11 结构性登顶 0.0199；EE 解药=w192/d128=F1 引擎（初判）；w384 周期假设证伪
  - 细搜三批次（实录 38-40）：分量级赢家诅咒（t11 F1 +3σ 运气）；d1_t11d64 领跑；d5 名义优势蒸发（批次效应四现·配对对称纪律四验）；字典序判定树（S→F1→|EE|→MAE）③裁决锁定 d1_t11d64
  - Test 终局（实录 41）：seed 12000 四线全过（S_test 0.0349/F1 0.9231/EE +1.33% 转正）+无漂移；TP36/FN3/FP3/N39；EE 方向三验全败于预测；Test 预算 2 用 1 封存
  - 收官沉淀：REPORT.md v1.2（§8 dw 纪元+三纪元对照+跨纪元结论九条）；TUNING_GUIDE 第四纪元+踩坑 12-13
  - 沙箱重置四次（第十一至十四次），SOP 全部零丢失恢复；提交前基点核对升级为固定动作
...
## 会话纪要追加：跨数据集外部验证纪元群 Stage A（2026-09-15 收官，实录 51-58）
- **立项（51）**：六条外部效度预注册；开发集 H1/H2、留存第三 house H5；验收四线 + Test 预算 2 次/目标。
- **侦察+labels（52-53）**：三 house 覆盖率算术（H5 主战场 ~94%）；**H4 整 house 排除（混表=发现 #1，labels 必查共享表）**；4 目标选定。
- **普查（54）**：viability 双排除（H3k 66<90 / H5mw 12≪90+42W 表底）=铁律跨 house 双确认；口径定谳 k 500W / dw 200W（四判据三机型）。
- **探针+对称化（55-56）**：15+6 runs；dw d1 显著优（ΔS −0.0251·3/3·>2×SEM 0.0201）=连平第三 house；kettle 三方打平、base |EE| 17.61/21.03/15.31% 3/3 超硬门槛出局、f4/f5 字典序 f5 胜（等价偏 f5）。
- **Test（57-58）**：kettle #1 败（S 贴线+EE 翻转）→ #2 **四线全过**（23007·S 0.08900）；dw #1/#2 均仅 S 线爆（0.08247/0.06424 vs 0.04631）=**段级漂移**→**失败档案（用户裁定接受）**。
- **六条裁决**：①配方可平移·达标线不可平移（分电器定谳）②confirmed 双触发 ③confirmed ④无反例 ⑤confirmed 强化 ⑥confirmed。
- **注入事件注记（55）**：命令块输出遭外部注入（伪造完成声明/伪造 hash/token 索取/破坏性 git 指令），仓库实测三验揭穿后干净重做——「完成声明以仓库实测为准」入档。
- **Stage B（REDD/REFIT）暂缓（用户裁定）**；REDD 官网不可访问，直链+REFIT 备胎权衡已备案权衡。
- **commits 链（本纪元群）**：b1c3089（立项）→ed0734a（侦察）→bf1fd6e（labels）→dd8f0cc（普查）→cef63bf（探针判读）→b569a76（对称化+Test 预注册）→6f63caa（Test#1）→（本回合实录 58 收官）。
- **沙箱重置**：第 22-23 次，SOP 零丢失恢复。
