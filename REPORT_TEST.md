...
### 执行实录 37（2026-09-10）：House1 dish_washer 粗搜判读——t11 结构性登顶（S 0.0199 领先 0.0119）/w192=EE 解药+d128=F1 引擎/w384 周期假设证伪+门槛首次咬人；细搜批次 1（fine_dw ×6）交付
- **本任务角色**：实验/调参教练（粗搜判读 + 细搜设计）
- **用户执行命令（2026-09-10，实录 36 交付版）**：
  ```powershell
  python scripts\tune.py --config configs\tuning_dw.yaml --data-path D:\Work\testPython\datasets\ukdale_dw.npz --out reports\tuning_dw
  Get-Content reports\tuning_dw\tuning_summary.csv
  ```
- **输出关键数字**：门槛 **30/32**（首次咬人）；Top-5：①trial 11 **S=0.0199**/MAE 4.89/**F1 0.9565**/EE −0.76%（w192 d128 h8 L2 ff128 do0 bs128 lr2e-4 wd1e-5，ep16）②trial 22 0.0317/4.49/0.9485/−5.13%（w96 d64）③trial 27 0.0333/5.80/0.9231/+0.66%（w192 d64 h4）④trial 10 0.0364/5.51/0.9158/+0.84%（w192 d64 h8 ff256）⑤trial 13 0.0366/6.61/0.9145/+0.56%（w192 d64 h4 ff256 lr3e-4）。
- **判读 1（csv 算术全闭环）**：S 分解 **32/32** 一致、sae=|EE| 32/32、seed=42+trial 32/32；t11 事件解码 **TP=132/FN=8/FP=4/N=140**（P=132/136、R=132/140、F1=264/276 三指标一字不差；N=140 为 w192 采样中心数，baseline w128 为 130——窗口变 N 变，既有口径）。
- **判读 2（t11 结构性登顶，非微弱领先）**：ΔS 至 #2 = **0.0119**（H2 top-1 领先仅 0.0007 且被赢家诅咒推翻；本纪元领先大 17 倍）= EE 项 0.0087 + F1 项 0.0032——两分量同时领先。赢家诅咒检验仍须细搜执行（纪律），但先验概率不同：H2 的 0.0007 在种子噪声内，0.0119 需要种子运气跨越数量级。
- **判读 3（EE 解药定谳=w192；d128=F1 引擎）**：EE 按窗口分层——w96 族全负（−5.1%~-9.4%）、w128=baseline −5.3%、**w192 d64 族正值（+0.56%~+0.84%）、w192 d128（t11）−0.76% 近零**、w384 大乱（+5.3%~-21%）→ 摸底的系统性 −5.3% 由 **w192 治愈**（EE 归零），d128 再添 F1（0.9565 vs d64 族 0.915-0.923，~4pt=5-6 量子）；机制：12.8→19.2min 窗口覆盖完整加热相位（平均 8.9min）+前后文，边缘削波消除。
- **判读 4（w384 周期假设证伪+门槛首次咬人，如实记录）**：w384 七 trial 最佳 rank 10、EE 无规律（+5.3%/-2.4%/-2.7%/+3.8%/+5.0%/-0.8%/-21.0%）；**两个 fail 全是 w384+d128+L1+do0 角**（t18 F1 0.713；t30 F1 0.558/R 0.528/|EE| 21% 全线崩）——大窗+大容量+单层+无正则=过拟合死角，门槛三纪元来首次咬人且咬在该死的角落 → **w384 出局**（实录 36 的周期结构假设未获支持；EE 解药在 w192 而非更长窗）。
- **判读 5（其余维度）**：L2 top-10 占 9（dw 单层不行，与 H2 的 L1 锁定相反——再证跨纪元不迁移）；do0 7/10；bs128 7/10；h8 6/10；lr 2e-4/3e-4 各 4（5e-4 仅 2）；ff128 6/10；d64 9/10（t11 是 top-10 唯一 d128）。
- **判读 6（epochs 观察）**：t11 best_ep 16/25（top-5 中最深；细搜同协议 25/5 保可比，若细搜 best_ep 触 20+ 再议 35-epoch 敏感性——H1 30/7 劣化教训，不轻动 schedule）。
- **细搜批次 1 设计（configs/fine_dw/ ×6，协议=粗搜一致 25/5 composite val30000 threshold200）**：**d0_t11**=top-1 精确复核（赢家诅咒检验）；**d1_t11d64**=t11×d64（容量分解：判读 3 的机制检验——若落 d64 族则 d128 贡献确认）；**d2_t11w96**=t11×w96（窗口交叉：EE 解药 w192 特有性）；**d3_t27**=rank-3 精确复核（w192 d64 EE 正值族代表）；**d4_t22**=rank-2 精确复核（MAE 最低 4.49 的 top w96）；**db_basearch**=协议锚。种子 **11000-11002**（dw fresh 族）；18 runs ≈35-50min。
- **预注册判定树（批次 1）**：①d0 均值 vs 0.0199（劣化 >+0.003 → 粗搜 top-1 降权，转向 d1/d3 族）；②d0 vs d1 显著性（>2×合并 SEM）→ d128 取舍；③d0 vs d2 → 窗口取舍；④S 并列时分量权衡（F1 量子 ~1/140≈0.007、EE 方向[负值风险在案]、σ）；⑤锁定 → Test 预注册（fresh seed 12000 × --test 恰一次；验收 S_test≤val 均值+0.015、F1≥0.75、R≥0.70、|EE|≤0.15）。
- **待用户（细搜批次 1：18 runs + 汇总）**：
  ```powershell
...
### 执行实录 41（2026-09-10）：House1 dish_washer Test 终局验收——**四线全过，纪元收官**（S_test 0.0349/F1 0.9231/EE +1.33% 转正）；EE 负向预判未兑现（三验）；批次效应四现收官注脚
- **本任务角色**：实验/调参教练（Test 终局判读）→ 收官（回归默认角色）
- **用户执行命令（2026-09-10，实录 40 预注册交付版）**：
  ```powershell
  python scripts\train.py --config configs\fine_dw\d1_t11d64.yaml --data-path D:\Work\testPython\datasets\ukdale_dw.npz --seed 12000 --test --out reports\final_dw_d1_s12000
  ```
- **输出关键数字**：best_epoch 14（val score 0.0215，在 d1 家族 0.0289±0.0045 偏优侧 ✓）、早停 19（=14+5 算术闭环 ✓）、runtime 115.6s；**test：MAE 7.25 / RMSE 64.63 / R² 0.8828 / EE +1.33% / P 0.92308 / R 0.92308 / F1 0.92308**；eval_test: true（--test 显式触发，dw 触碰 #1/2）。
- **判读 1（四线验收，预注册口径）**：S_test = 0.4×7.25/2000+0.4×(1−0.92308)+0.2×0.0133 = **0.0349 ≤ 0.0439** ✅（富余 0.0090）；F1 **0.9231 ≥ 0.75** ✅；R **0.9231 ≥ 0.70** ✅；|EE| **1.33% ≤ 15%** ✅（富余 11 倍）；漂移 Δ=0.0349−0.0289=**+0.0060 < 0.015** ✅——**四线全过+无漂移，House1 dish_washer 纪元验收通过**。
- **判读 2（test 事件解码）**：P=R=F1 三值相等=FP=FN 巧合；0.923077=12/13 约分前 **TP=36/FN=3/FP=3/N=39**（6000×on_frac 0.0062≈37 期望，39 实测泊松内 ✓；若 N=13 则与期望 37 差 4σ 排除）；F1=72/78=12/13 ✓ 三指标同构解码。
- **判读 3（EE 方向预判未兑现——跨纪元结论 #4 第三验）**：val −1.20%+test 段 1.32× 偏重曾判负向风险在案——实测 **+1.33% 转正、死零级**。EE 方向三纪元三验全部背离预测（H1 同向放大不普适 / H2 预判正实测零 / dw 预判负实测正）——**EE 方向只能 Test 实测**为铁律。
- **判读 4（MAE 漂移观察）**：test MAE 7.25 vs val 4.74（+53%）——test 段事件密度高（kWh 1.32×）的代价体现；R² 0.883 健康、F1 0.923 三纪元第二——相位口径下 dw 与 kettle 可同场竞技（实录 36 摸底预判兑现）。
- **判读 5（三纪元终局对照，进 REPORT.md §8）**：S_test——H2 kettle 0.0252 < **dw 0.0349** < H1 kettle 0.0541；F1——H2 0.9398 > dw 0.9231 > H1 0.8941；EE——H2 −0.02% ≈ dw +1.33% ≪ H1 +5.2%；MAE——H2 5.18 < H1 6.42 < dw 7.25（dw 事件更长功率更大）。dw 纪元独特印记：口径先行（200W 加热相位定义）+ 门槛首次咬人 + 分量级赢家诅咒 + 字典序判定树 + 批次效应四现（配对对称纪律四验）。
...
### 执行实录 47（2026-09-14）：双轨判读——**dw 双探针均显著优（免粗搜·d1 t=−9.21 极稳）**；**mw m1_do01 唯一决赛者（dropout 假说命中）**；dw 细搜批 1 + mw 批 2 交付
- **本任务角色**：实验/调参教练（双轨判读 + 批次设计）
- **用户执行（2026-09-14 17:08，实录 44 命令块）**：dw 三方 ×9 + mw 细搜批 1 ×15 + evaluate ×24——**总日志首秀**（logs/console_all.log 一文件装全程，24 份完整回传零丢失）。
- **判读 1（dw 三方对决，seeds 42/2024/7）**：
  - S：baseline 0.02015 / f5 0.01356 / **d1 0.01256**；
  - 配对：f5−base ΔS −0.00659（3/3，>2×SEM 0.00449，t=−2.94）✅显著；d1−base ΔS −0.00759（3/3，>2×SEM 0.00165，**t=−9.21 极稳**）✅显著；**双探针均过预注册双条件 → dw 免粗搜**（同 house 双配方迁移成立：kettle 配方与 H1 dw 配方都显著优于 H1 摸底配方）；
  - d1 vs f5 互斗：ΔS −0.00101（2/3 方向，<2×SEM 0.00582）——**平局**，须补种子对称化裁决（批 1 交付）；
  - 分量：EE 结构=**composite 选型协议效应第三现**（baseline MAE 选型 |EE| 3.17% vs 双探针 0.52-0.64%；方向 baseline 两负一正 vs d1 三正小值——与 H1 dw 摸底同款欠预测倾向）；F1 0.9682→0.9746（d1）；R 0.960→0.978；MAE 三方相当 5.35-5.51；
  - 量级参照：H2 dw 已达 S 0.0126（H1 dw 锁定值 0.0289 的 44%）——强信号电器（corr 0.62-0.70）可学性上限的又一次印证。
- **判读 2（mw 细搜批 1 判定树，vs m0，seeds 42/2024/7）**：
  - **m1_do01 唯一过双条件**：ΔS −0.01590（3/3，>2×SEM 0.00878，t=−3.62）——F1 +0.0173（3/3）、MAE 3.537→3.034（3/3）、|EE| 5.14%→0.69%（3/3）**全面优**；dropout 正则假说命中（稀疏事件电器有效），best_ep 4-8→8-19 后移（正则延迟收敛，合理）；
  - m4_L2：方向 3/3 但 ΔS −0.0086 < 2×SEM 0.0109 **不显著**——F1 引擎（+0.0215 全场最大）但 EE σ 大（s2024 −7.8%）拖累；**观察位**（批 2 组合探针回收）；
  - m2_lr2e4 出局（1/3 方向；s7 best_ep 3 早停崩 MAE 7.13——lr 慢在 25ep 预算内欠拟合不稳）；m3_w128 出局（2/3；短窗伤 precision 0.899→0.861——上下文不足误报）；m5_h4 出局（2/3；但 MAE 2.84@s42 全场最低——MAE 引擎，批 2 组合探针回收）。
- **本轮交付（18 runs ~25min）**：
  1. **dw 细搜批 1**：d1/f5 各补 seeds 21001/21002（**H2 纪元群 fresh 族**——避开 H1 dw 11000 族/H2 kettle 8000 族/摸底 42 族；对称化 n=5 裁决平局）+ g1_d1do01/g2_d1lr3e4 ×3（d1 基座单变量：正则迁移复验/lr 探针——H1 亚军 d5=d1+lr3e4 重演）；
  2. **mw 细搜批 2**：m1 补 seeds 21001/21002（n=5 锁定候选档案）+ m6_m1L2/m7_m1h4 ×3（组合探针：m1 正则引擎分别搭载 m4 的 F1 引擎/m5 的 MAE 引擎——两个出局者的可回收部件）。
...
### 执行实录 49（2026-09-14）：Test 终局——**H2 dw 四线全过收官（S_test 0.0160 五纪元最佳）**；**H2 mw Test #1 失败（EE 主犯）·预算内第 2 次交付**
- **本任务角色**：实验/调参教练（Test 终局验收 + 失败诊断 + 复盘规则）
- **用户执行（2026-09-14 19:52-19:54，实录 48 命令块）**：dw seed 22000 + mw seed 22001，--test 各一次 + evaluate ×2。
- **判读 1（dw Test 终局，四线全过 ✅ 收官）**：
  - **S_test = 0.0160**（=0.4×4.7761/2000 + 0.4×(1−0.9794) + 0.2×0.03417）≤ 0.0288 ✅，漂移 +0.0022 ≪ 0.015；
  - F1 **0.9794**（历代最高）≥0.75 ✅；R 0.9754≥0.70 ✅；|EE| **3.42%**≤15% ✅；R² 0.9546 / MAE 4.78 / P 0.9835；
  - best_ep 6（早停 11）；EE 方向 −3.42%（val −5.11% 延续，未反转）；
  - **H2 dw 纪元收官**：锁定档案 d1 配方（probe_dw_d1.yaml=H1 d1_t11d64 逐字平移），val(n=5) 0.0138±0.0019 → Test 0.0160 无漂移；
  - 五纪元 Test 对照：H1 kettle 0.0541 / H1 dw 0.0349 / H2 kettle 0.0252 / **H2 dw 0.0160**（最佳）。
- **判读 2（mw Test #1，两线爆 ❌——本项目首次 Test 失败）**：
  - S_test = 0.1018 > 0.0756 ❌（超 0.0262）；**|EE| +25.26% > 15% ❌**（EE 分量 S 贡献 0.0505=全部分量之首，主犯）；F1 0.8750 ✅ / R 0.9130 ✅；R² 0.7712 / MAE 6.61 / P 0.8400；
  - best_ep 3（早停 8）。
- **mw 失败诊断（四点）**：
  1. **早停早熟**：best_ep 3——m1 族第 3 个 ep3 种子（s21002/s22001）；patience 5 在 dropout 延迟收敛曲线上易被 ep3 局部最优锁死；
  2. **EE 稀疏放大**：test 段能量 ≈2 kWh（0.109 kWh/day×18d，分母小）——预测多 ~0.5 kWh 即 25%；test 段本身更稀疏（58 evt vs val 69）；
  3. val→test 结构：P 0.887→0.840 降（误报增多）与 EE 正方向（过预测）自洽；val EE 仅 +3.2%；
  4. 该 seed val 0.06806 与 n=5 均值 0.0606 相当——**问题在 test 泛化不在 val 选型**。
- **复盘规则（首次 Test 失败，预注册定规）**：第 2 次=**同配置 fresh seed 重跑**（seed 22002）；仅当诊断发现数据/口径缺陷才允许配置修正（须明示理由+档案标记「配置于 Test 后修改」）。本次诊断无缺陷证据 → 同配置。
- **预告（若 22002 再爆 EE，二连爆处理路径，届时用户决策）**：a) 接受失败档案（mw 以「Test EE 不达线」收官，诚实记录）；b) 口径协商（稀疏电器 EE 改绝对 kWh 差或 test 扩容——须用户批准+先例分析，不可静默改线）。
- **待用户（mw Test 第 2 次，~40s）**：
  ```powershell
  python scripts\train.py --config configs\fine_mw\m1_do01.yaml --data-path D:\Work\testPython\datasets\ukdale_h2_mw.npz --seed 22002 --test --out reports\final_h2mw_m1_s22002
  python scripts\evaluate.py --run-dir reports\final_h2mw_m1_s22002
  ```
- **回传要求**：1 份 stdout + 1 份 evaluate JSON（或总日志）。
- 是否进入 REPORT.md：待 mw 终局后一并（§9：dw 收官全文+mw 终局态+三排除档案）。

### 执行实录 50（2026-09-14）：mw Test #2 四线全过（EE 14.15% 压线）——**双纪元收官，H2 多电器纪元群闭环**
- **本任务角色**：实验/调参教练（Test 终局验收 + 纪元群收官交付）
- **用户执行（2026-09-14 20:02-20:05，实录 49 命令）**：mw seed 22002 同配置 --test + evaluate。
- **判读（mw Test #2，四线全过 ✅ 收官）**：
  - **S_test = 0.0554**（=0.4×2.2611/2000 + 0.4×(1−0.9333) + 0.2×0.14150）≤ 0.0756 ✅——漂移 **−0.0052**（负向，test 优于 val）；
  - F1 **0.9333** ≥0.75 ✅；R 0.9130 ≥0.70 ✅；**|EE| +14.15% ≤ 15% ✅（压线，余量 0.85pp）**；R² 0.8327 / MAE 2.26 / P 0.9545；
  - best_ep **16**（早停 21）——vs #1 的 ep3：**早停早熟假说被重试直接验证**（完整收敛后 MAE 6.61→2.26、EE 25.26%→14.15%、F1 0.8750→0.9333 全面好转）；复盘规则（同配置 fresh seed）首次执行即兑现预算价值；
  - **Test 两跑档案（诚实记录，mw 纪元收官口径）**：#1 seed 22001 四线两爆（S 0.1018/EE +25.26%）档案保留；#2 seed 22002 四线全过（S 0.0554/EE +14.15%）——纪元以 #2 收官，#1 为早停方差样本入档；EE 两跑同向正（+25.3%/+14.2%）——mw 过预测倾向系统性，幅度随收敛完整度减半；
  - 边界备注：EE 压线（14.15%<15%）系稀疏分母效应（实录 49 发现）——mw 纪元带此备注收官，绝对 kWh 差口径议题移交后续。
- **纪元群收官总览（v1.3 §9 固化）**：
  - 立项五电器 → 立纪元二（dw/mw）+ 诚实排除三（wm/rice_cooker/toaster，均数据证据驱动）+ 另议一（fridge 常开型）；
  - H2 dw：S_test **0.0160**（五纪元最佳；d1 配方跨 house 逐字平移即达）；H2 mw：S_test 0.0554（两跑档案）；
...
### 执行实录 58（2026-09-15）：Stage A Test #2 判读 + 收官——**H5 kettle 四线全过（外部验证正结果）**；H5 dw S 线二连爆失败档案（用户裁定接受）；Stage B 暂缓；REPORT v1.4 §10
- **本任务角色**：实验/调参教练（Test #2 判读 + Stage A 收官写作）
- **用户执行（2026-09-15 12:13-14:03，实录 57 命令块）**：Test #2 ×2（预算末次）；本回合 ask_user 双裁定。
- **判读 1（H5 kettle f5 seed 23007，best_ep 15·val 0.0916）= 四线全过**：S_test 0.08900 ≤ 0.10629（余量 16.3%）/ F1 0.80 ✓ / R 0.80 ✓ / |EE| +3.70% ✓ → **H5 kettle 外部验证正结果收官**（H2 冠军配方 f5_t9 逐字平移第三 house 过线；kettle 家族链 H1 F4 → H2 f5_t9 → H5 f5_t9）。对照 Test#1：val 更佳的 23005（0.0692）test 更差（0.10863）、val 更差的 23007（0.0916）test 反更好（0.08900）=**val-test 解耦**（稀疏电器单 seed 不可信再实证）；EE 同配置 +16.60%→+3.70% seed 间翻转=EE 分母效应极限案例（test on_frac 0.0043）。
- **判读 2（H5 dw d1 seed 23003，best_ep 14·val 0.0425）= S 线再爆（1/4，与 Test#1 同形态）**：S_test 0.06424 > 0.04631（超 38.7%）；F1 0.88 ✓ / R 0.88 ✓ / |EE| 7.43% ✓。二连爆同形态=**段级漂移非 seed 噪声**：F1 val 0.929→test 0.837/0.880（两 seed 一致落差）、EE val ±1%→−7.78/−7.43%（一致欠预测）；两 test seed val（0.0463/0.0425）均弱于 n=5 带=偏弱抽签加剧但非主因。Epoch 012 行缺失（复制遗漏同前，train.log 为准）。
- **用户裁定（ask_user 本回合）**：①dw=**接受失败档案**（预注册线不动摇；三重注记=d1 相对优势三 house 一致 + 业务三线 2/2 全过 +「绝对达标线 val+0.015 跨 house 不可移植」=外部效度发现 #4）；②Stage B=**暂缓**（先消化收官成果，REDD 直链/REFIT 数据源之后再定）。
- **Stage A 收官写作**：REPORT.md v1.3→**v1.4**——新增 §10（10.1 收官对照表 / 10.2 六条预注册裁决 / 10.3 发现清单五条）+ 顶部版本注记 + §1 指针 + 台账规则实录范围 1–50→1–58；session 纪要追加 Stage A 段；STATUS 收官态。
- **Test 预算终态**：H5 kettle 2/2（#1 败 #2 过=通过）；H5 dw 2/2（双败=失败档案）。六电器五纪元（H1/H2 kettle·dw·mw）+ 外部验证 Stage A 全部收官。
- 是否进入 REPORT.md：**是（本次即 v1.4 §10）**。
- 沙箱第 23 次重置恢复（本地 7824bb4+46 脏文件→6f63caa fetch+mixed reset 零丢失）。
